...
class MoviedbDatasource extends MoviesDatasource { // Se crea una instancia de Dio con la configuración base de la API. 
...
// Hace la petición HTTP a la API, transforma los datos y devuelve una lista de Movie.
  ...
  Future<List<Movie>> getNowPlaying({int page = 1}) async {
    final response = await dio.get('/movie/now_playing'); // Llamada a la API
    final movieDBResponse = MovieDbResponse.fromJson(response.data); // Se parsea la respuesta
